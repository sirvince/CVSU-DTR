...
The repo is now under git version control (`git init` + an initial commit capturing the full MVP as of 2026-08-20; no remote configured yet — that's the user's call). Target topology, confirmed with the user: **managed split-hosting** — managed MySQL (e.g. Aiven) + the API on a container host (Render/Fly/Railway-style) + the frontend on a static host (Cloudflare Pages/Netlify/Vercel-style). This is a different shape than docs/stack(1).md §13's self-hosted "Docker Compose + Nginx on one box" sketch — that doc predates this decision and wasn't updated, since it's describing one reasonable option, not a requirement.
...
- **No SPA fallback routing for the static-hosted frontend.** `apps/web` uses React Router's `BrowserRouter` (path-based, not hash-based) — a static host with no fallback config 404s on a hard refresh/direct link to any route but `/`. Added `apps/web/public/_redirects` (`/* /index.html 200`), the format Cloudflare Pages and Netlify both read natively; Vite copies `public/` into `dist/` on build, confirmed present in the build output. **If the frontend ends up on Vercel instead, this file does nothing there — Vercel needs a `vercel.json` rewrite instead.**
...
- **Generated-file persistence.** `STORAGE_PATH` (`storage/generated`) writes `.xlsx` files to the container's own filesystem. Most managed container hosts give the app container an *ephemeral* disk — files written there don't survive a redeploy or restart unless you explicitly attach a persistent volume (Render and Railway both offer one, at extra cost/config). Practical impact is limited (a teacher can always call `POST /dtr/generate` again to reproduce the file — `dtr/calendar`'s data isn't lost, only the previously-generated `.xlsx` bytes are), but a `GET /dtr/generate/:id/download` for an old `DtrGeneration` row would 404 after a redeploy wipes the disk. Decide when picking the actual host: attach a persistent volume, or accept "regenerate if you need an old version" as the MVP behavior (matches DTR-014/History already being explicitly out of MVP scope).
