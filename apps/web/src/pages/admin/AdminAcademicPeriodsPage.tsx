...
// Admin-only full CRUD over the shared academic-period list — the same
// list every teacher sees read-only at pages/AcademicPeriodsPage.tsx
// (/academic-periods). Reuses the exact same api/academic-periods.ts
// functions and Zod schema; the backend enforces write access is
// admin-only (RolesGuard), this page is just the UI that can reach the
// write endpoints at all (only linked from the nav when user.role ===
// 'ADMIN' — see components/Layout.tsx — and the /admin/academic-periods
// route itself is wrapped in RequireAdmin in App.tsx).
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Shared across every teacher — create the terms teachers will pick from."
      />
